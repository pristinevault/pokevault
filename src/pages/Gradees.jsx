...
export default function Gradees({ gradees, userId, onRefresh }) {
  ...
  const filtered = useMemo(() => {
    let data = [...gradees]
    if (filters.search) data = data.filter(g => (g.pokemon || '').toLowerCase().includes(filters.search.toLowerCase()) || (g.serie || '').toLowerCase().includes(filters.search.toLowerCase()))
    if (filters.gradeur) data = data.filter(g => g.gradeur === filters.gradeur)
    if (filters.note) data = data.filter(g => g.note === filters.note)
    if (filters.serie) data = data.filter(g => (g.serie || '').toLowerCase().includes(filters.serie.toLowerCase()))
    return data
  }, [gradees, filters])
  ...
  function handleBrowserSelect(cardData) {
    setForm({ ...EMPTY, gradeur: 'PSA', pokemon: cardData.pokemon || '', serie: cardData.serie || '', numero: cardData.numero || '', image_url: cardData.image_url || '' })
    setImagePreview(cardData.image_url || null)
    setImageFile(null); setBrowser(false); setModal(true)
  }
  ...
  async function uploadImage(currentUrl) {
    if (!imageFile) return currentUrl || null
    setUploading(true)
  ...
  async function handleSave() {
    if (!form.gradeur || !form.note) { alert('Merci de sélectionner un gradeur et une note.'); return }
    setSaving(true)
    const imageUrl = await uploadImage(form.image_url || editing?.image_url || null)
    const payload = {
      pokemon: form.pokemon || null, serie: form.serie || null, numero: form.numero || null,
    ...
    const result = editing
      ? await supabase.from('gradees').update(payload).eq('id', editing.id)
      : await supabase.from('gradees').insert(payload)
    setSaving(false)
    if (result.error) { alert(`Erreur : ${result.error.message}`); return }
    setModal(false); setEditing(null); setForm(EMPTY); setImageFile(null); setImagePreview(null); onRefresh()
  }
  ...
  // Notes uniques dans la collection
  const notesUniques = [...new Set(gradees.map(g => g.note).filter(Boolean))].sort()
  const seriesUniques = [...new Set(gradees.map(g => g.serie).filter(Boolean))].sort()
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 2 }}>{filtered.length} cartes · PA {formatEur(totaux.pa)} · Valeur {formatEur(totaux.val)}</div>
        </div>
      ...
      {/* Filtres */}
      <div className="filters-row" style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
        <input placeholder="🔍 Pokémon, série..." value={filters.search || ''} onChange={e => setF('search', e.target.value)} style={{ minWidth: 180 }} />
        <select value={filters.serie || ''} onChange={e => setF('serie', e.target.value)} style={{ minWidth: 140 }}>
          <option value="">Toutes séries</option>
          {seriesUniques.map(s => <option key={s} value={s}>{s}</option>)}
        </select>
        <select value={filters.gradeur || ''} onChange={e => setF('gradeur', e.target.value)} style={{ minWidth: 120 }}>
          <option value="">Tous gradeurs</option>
          {Object.keys(GRADEURS).map(g => <option key={g} value={g}>{g}</option>)}
        </select>
        <select value={filters.note || ''} onChange={e => setF('note', e.target.value)} style={{ minWidth: 110 }}>
          <option value="">Toutes notes</option>
          {notesUniques.map(n => <option key={n} value={n}>{n}</option>)}
        </select>
        {Object.values(filters).some(v => v) && (
          <button className="btn-ghost" onClick={() => setFilters({})} style={{ padding: '7px 12px', fontSize: 12 }}>✕ Reset</button>
        )}
      </div>

      {/* Grille */}
      <div className="grid-cards" style={{ display: 'grid', gap: 14 }}>
        {filtered.map(g => {
          ...
          return (
            <div key={g.id} className="card" style={{ padding: 0, overflow: 'hidden' }}>
              <div style={{ position: 'relative', background: 'var(--bg-elevated)', minHeight: 160, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 10 }}>
                {g.image_url
                  ? <img src={g.image_url} alt={g.pokemon} style={{ maxHeight: 170, maxWidth: '100%', objectFit: 'contain', borderRadius: 6 }} onError={e => e.target.style.display = 'none'} />
        ...
        {!filtered.length && (
          <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: 60, color: 'var(--text-muted)' }}>
            ...
            {gradees.length === 0 ? 'Aucune carte gradée.' : 'Aucun résultat pour ces filtres.'}
          </div>
        )}
      </div>
          ...
          <div className="modal" style={{ maxWidth: 560 }}>
            ...
            <div style={{ display: 'flex', gap: 16, marginBottom: 16 }}>
              <div style={{ flexShrink: 0 }}>
                <div style={{ width: 100, height: 140, borderRadius: 8, border: '2px dashed var(--border)', background: 'var(--bg-elevated)', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
                  {imagePreview ? <img src={imagePreview} style={{ width: '100%', height: '100%', objectFit: 'contain' }} onError={() => setImagePreview(null)} /> : <span style={{ fontSize: 30, opacity: 0.2 }}>🃏</span>}
                  {form.gradeur && form.note && <div style={{ position: 'absolute', bottom: 4, right: 4 }}><GradeBadge gradeur={form.gradeur} note={form.note} size="small" /></div>}
                </div>
                <input type="file" accept="image/*" onChange={handleImageChange} style={{ fontSize: 10, marginTop: 6, width: 100 }} />
              </div>
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div><label style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 0.5, display: 'block', marginBottom: 4 }}>Pokémon</label><input value={form.pokemon || ''} placeholder="ex: Dracaufeu" onChange={e => set('pokemon', e.target.value)} /></div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                  <div><label style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 0.5, display: 'block', marginBottom: 4 }}>Série</label><input value={form.serie || ''} placeholder="ex: SFAFR" onChange={e => set('serie', e.target.value)} /></div>
                  <div><label style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 0.5, display: 'block', marginBottom: 4 }}>Numéro</label><input value={form.numero || ''} placeholder="ex: 75/64" onChange={e => set('numero', e.target.value)} /></div>
                </div>
              </div>
            </div>
            <div style={{ padding: '12px 14px', background: 'var(--bg-elevated)', borderRadius: 10, marginBottom: 14 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <div><label style={{ fontSize: 11, color: 'var(--text-muted)', display: 'block', marginBottom: 4 }}>Société</label>
                  <select value={form.gradeur} onChange={e => { set('gradeur', e.target.value); set('note', '') }}>{Object.keys(GRADEURS).map(g => <option key={g} value={g}>{g}</option>)}</select></div>
                <div><label style={{ fontSize: 11, color: 'var(--text-muted)', display: 'block', marginBottom: 4 }}>Note</label><GradeSelect gradeur={form.gradeur} note={form.note} onNoteChange={v => set('note', v)} /></div>
              </div>
              {form.gradeur && form.note && <div style={{ marginTop: 10, display: 'flex', justifyContent: 'center' }}><GradeBadge gradeur={form.gradeur} note={form.note} /></div>}
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 14 }}>
              {[['P.A. (€)', 'prix_achat'], ['Valeur gradée (€)', 'valeur'], ['Valeur loose (€)', 'loose']].map(([label, key]) => (
                <div key={key}><label style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 0.5, display: 'block', marginBottom: 4 }}>{label}</label><input type="number" step="0.01" value={form[key] || ''} onChange={e => set(key, e.target.value)} /></div>
              ))}
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 18 }}>
              <div><label style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 0.5, display: 'block', marginBottom: 4 }}>Date de gradage</label><input type="date" value={form.date_gradage || ''} onChange={e => set('date_gradage', e.target.value)} /></div>
              <div><label style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 0.5, display: 'block', marginBottom: 4 }}>Notes</label><input value={form.notes || ''} placeholder="Optionnel" onChange={e => set('notes', e.target.value)} /></div>
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
              <button className="btn-ghost" onClick={() => setModal(false)} style={{ flex: 1 }}>Annuler</button>
              <button className="btn-primary" onClick={handleSave} disabled={saving || uploading} style={{ flex: 2 }}>{saving || uploading ? '⟳ Sauvegarde...' : editing ? 'Mettre à jour' : 'Ajouter'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {deleting && (
        <div className="modal-overlay"><div className="modal" style={{ maxWidth: 340, textAlign: 'center' }}>
